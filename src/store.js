...
const useStore = create((set, get) => ({
  ...
  addWall: (p) => { 
    ...
    set(s => ({ 
      walls: [...s.walls, { 
        id, 
        start: p, end: p, 
        thickness: 10, height: 100, 
        levelId: s.currentLevelId, 
        colorSideA: '#9ca3af', colorSideB: '#9ca3af', 
        textureA: 'none', textureB: 'none', // <--- New Fields
        openings: [] 
      }], 
      selectedWallId: id 
    })); 
  ...
  // NEW: Update Wall Texture
  updateWallTexture: (id, side, texture) => {
    get().saveHistory();
    set(s => ({
      walls: s.walls.map(w => {
        if (w.id !== id) return w;
        if (side === 'A') return { ...w, textureA: texture };
        if (side === 'B') return { ...w, textureB: texture };
        return w;
      })
    }));
  },
  ...
  splitWallAtPoint: (wid, p) => { 
    ...
    const baseProps = { ...w, id: undefined, start: undefined, end: undefined, openings: [] };
    const w1 = { ...baseProps, id: uuidv4(), start: w.start, end: p };
    const w2 = { ...baseProps, id: uuidv4(), start: p, end: w.end };
    set(s => ({ walls: s.walls.filter(x=>x.id!==wid).concat([w1, w2]) }));
  },

  // UPDATED: addPerimeterWalls with textures
  addPerimeterWalls: (lid) => { 
    const { siteWidth, siteDepth } = get(); if (!siteWidth) return; 
    const p = { thickness: 10, height: 100, levelId: lid, colorSideA: '#9ca3af', colorSideB: '#9ca3af', textureA: 'none', textureB: 'none', openings: [] }; 
    set(s => ({ walls: [...s.walls, { id: uuidv4(), start: {x:0,y:0}, end: {x:siteWidth,y:0}, ...p }, { id: uuidv4(), start: {x:siteWidth,y:0}, end: {x:siteWidth,y:siteDepth}, ...p }, { id: uuidv4(), start: {x:siteWidth,y:siteDepth}, end: {x:0,y:siteDepth}, ...p }, { id: uuidv4(), start: {x:0,y:siteDepth}, end: {x:0,y:0}, ...p }] })); 
  },
  ...
  finalizeWall: (activeId) => {
    ...
    if (splitPoints.length > 0) {
        ...
        const baseProps = { ...active, id: undefined, start: undefined, end: undefined }; 
        let newSegments = [];
        splitPoints.forEach(pt => { newSegments.push({ ...baseProps, id: uuidv4(), start: currentStart, end: pt }); currentStart = pt; });
        newSegments.push({ ...baseProps, id: uuidv4(), start: currentStart, end: active.end });
        set(state => ({ walls: state.walls.filter(w => w.id !== activeId).concat(newSegments), selectedWallId: null }));
    }
  },
  ...
  // 5. PROJECT IO
  loadProject: (data) => {
    if (!data.walls || !data.levels) { alert("Invalid Project File"); return; }
    set({
      walls: data.walls, furniture: data.furniture || [], roomLabels: data.roomLabels || [], stairs: data.stairs || [],
      levels: data.levels || [{ id: 0, name: 'Ground', elevation: 0, height: 100 }],
      siteWidth: data.siteWidth, siteDepth: data.siteDepth, currentLevelId: data.currentLevelId || 0,
      past: [], future: [], selectedWallId: null
    });
  },
